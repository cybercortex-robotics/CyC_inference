...
#define BRUTEFORCE_DBSCAN false
 ...
 */
template <typename T>
class DBSCANSample final 
{
public:
    /***
     * Default constructor not supplied here
     */
    DBSCANSample() = default;
     ...
     */
    DBSCANSample(const Feature<T>& feature, uint class_id = CLASSIFY_FLAGS::NOT_CALSSIFIED);
...
template <typename T>
DBSCANSample<T>::DBSCANSample(const Feature<T> &feature, uint class_id)
{
    _m_feature = feature;
...
template <typename T>
T DBSCANSample<T>::operator[](int idx) const 
{
...
template <typename T>
Feature<T> DBSCANSample<T>::get_feature_vector() const 
{
    return _m_feature;
}

template <typename T>
void DBSCANSample<T>::set_feature_vector(const Feature<T> &feature)
{
...
template <typename T>
uint DBSCANSample<T>::get_class_id()
{
    return _m_class_id;
}

template <typename T>
void DBSCANSample<T>::set_class_id(uint class_id)
{
    _m_class_id = class_id;
}
