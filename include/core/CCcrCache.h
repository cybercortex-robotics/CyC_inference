...
{
public:
    using key_t = CCR_TIME_UNIT;

    CCcrCache()
        : m_size(CACHE_SIZE)
    {}

    CCcrCache(size_t size)
        : m_size(size)
    {}

    CCcrCache(const CCcrCache&) = default;
    CCcrCache(CCcrCache&&) noexcept = default;
    CCcrCache& operator=(const CCcrCache&) = default;
    CCcrCache& operator=(CCcrCache&&) noexcept = default;
    ~CCcrCache() = default;

    template <typename value_t>
    typename std::enable_if<!std::is_same<CcrOcTree, value_t>::value, void>::type
    insert(key_t key, const value_t& value, std::unordered_map<CcrDatablockKey, CCR_TIME_UNIT> sync = std::unordered_map<CcrDatablockKey, CCR_TIME_UNIT>())
    {
    ...
    const std::unordered_map<CcrDatablockKey, key_t>& sync(key_t key) const
    {
...
private:
    ...
    std::unordered_map<key_t, std::unordered_map<CcrDatablockKey, key_t>> m_cache_inputs;
};
