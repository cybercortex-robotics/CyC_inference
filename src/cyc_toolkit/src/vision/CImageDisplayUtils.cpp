    ...
    const bool& _show_curr_data)
{
    ...
    for (size_t i = 0; i < _slam_data.prev_poses_Body_W.size(); ++i)
    {
        ...
        draw_pose(_out_dst, cam_position.x(), cam_position.y(), prev_body_W.rotation_euler().z(), _bew_scale, color, 2);
    ...
    if (_draw_curr_frame)
    {
        // Draw body frame
        if (_absolute_coord)
            draw_pose(_out_dst, _slam_data.Absolute_Body_W.translation_3x1().x(),
                _slam_data.Absolute_Body_W.translation_3x1().y(),
                _slam_data.Absolute_Body_W.rotation_euler().z(), _bew_scale, color::blue, 4);
        else
            draw_pose(_out_dst, 0.f, 0.f, _slam_data.Absolute_Body_W.rotation_euler().z(), _bew_scale, color::blue, 4);

        // Draw IMU frame
        if (_absolute_coord)
            draw_pose(_out_dst, _slam_data.Absolute_Imu_W.translation_3x1().x(),
                _slam_data.Absolute_Imu_W.translation_3x1().y(),
                _slam_data.Absolute_Imu_W.rotation_euler().z(), _bew_scale, color::pink, 4);
        else
            draw_pose(_out_dst, 0.f, 0.f, _slam_data.Absolute_Imu_W.rotation_euler().z(), _bew_scale, color::pink, 4);
    }
