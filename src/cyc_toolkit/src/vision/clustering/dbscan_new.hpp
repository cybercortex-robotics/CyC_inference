...
auto run(const std::vector<std::pair<float, float>>& data, float eps, int min_pts)
{
    eps *= eps;
    ...
    for(size_t i = 0; i < data.size(); i++)
    {
        ...
        const float query_point[2] = { data[i].first, data[i].second };
        index.radiusSearch(query_point, eps, matches, SearchParams(32, 0.f, false));
        ...
        while (matches.empty() == false)
        {
            ...
            const float query_point_nb[2] = { data[nb_idx].first, data[nb_idx].second };
            index.radiusSearch(query_point_nb, eps, sub_matches, SearchParams(32, 0.f, false));
